...
public class datareader extends HttpServlet {
			...
			while (line != null) {
				...
				if (line.length() > 0 && line.charAt(0) == 'q') {
					...
					if (line != null) {
						...
						out.print("					<tr>");
						out.print("						<td style=\"padding: 10px;\">" + line + "</td>");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		out.print("				background-color: #3366ff");
		...
		for (int i = 0; i < num; i++) {
			try {
				...
				// write to some file here... here is my pseudocode. You should
				// overwrite the file each time you submit so that there is only
				// one game at a time.
				...
				for (int i = 0; i < num; i++) {
					try {
						...
						if (currentrow == r && currentcol == c) {
							out.print("					<td><font color=\"white\">" + request.getParameter("score" + i) + "</font></td>");
						}
					} catch (Exception e) {
